...
def menu():
    choices = ["Add new cards", "Delete cards", "Print Catalogue", "Exit"]
...
def add_card():
    ...
    card_strength = int(card_strength) if int(card_strength) <= 25 else 25
    card_speed = int(card_speed) if int(card_speed) <= 25 else 25
    card_stealth = int(card_stealth) if int(card_stealth) <= 25 else 25
    card_cunning = int(card_cunning) if int(card_cunning) <= 25 else 25

    catalogue[card_name] = {
        "Name": card_name,
        "card_strength": card_strength,
        "card_speed": card_speed,
        "card_stealth": card_stealth,
        "card_cunning": card_cunning
    }
...
def delete_card():
    ...
    if card_name in catalogue:
        ...
        easygui.msgbox("Deleted {} from the catalogue.".format(card_name))
    else:
        easygui.msgbox("No card named {} found in the catalogue."
                       .format(card_name))
