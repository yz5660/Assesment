...
def menu():
    choices = ["Add new cards", "Delete cards", "Edit", "Print Catalogue",
               "Output All", "Exit"]
    choices = easygui.buttonbox("Choose an option:", choices=choices)
...
def add_card():
    ...
    catalogue[card_name] = {
        "Name": card_name,
        "card_strength": min(int(card_strength), 25),
        "card_speed": min(int(card_speed), 25),
        "card_stealth": min(int(card_stealth), 25),
        "card_cunning": min(int(card_cunning), 25)
    }
...
def search_card():
    search = input("Enter the name of the monster card: ")
    match_cards = []
